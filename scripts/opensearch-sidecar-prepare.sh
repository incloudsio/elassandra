#!/usr/bin/env bash
# Sync Elassandra sources into the OpenSearch side-car and apply all patch scripts (no Gradle).
# Used by opensearch-sidecar-compile-try.sh and opensearch-sidecar-test-try.sh.
#
# Usage: ./scripts/opensearch-sidecar-prepare.sh [OPEN_SEARCH_CLONE_DIR]
#        or OPENSEARCH_CLONE_DIR=... ./scripts/opensearch-sidecar-prepare.sh
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
DEST="${1:-${OPENSEARCH_CLONE_DIR:-$ROOT/../incloudsio-opensearch}}"

"$ROOT/scripts/sync-elassandra-to-opensearch-sidecar.sh"
"$ROOT/scripts/sync-elassandra-fork-minimal-to-opensearch-sidecar.sh" "$DEST"
rm -f "$DEST/server/src/main/java/org/opensearch/index/mapper/DocumentParserCompat.java"
"$ROOT/scripts/sync-elassandra-fork-overlay-to-opensearch-sidecar.sh" "$DEST"
"$ROOT/scripts/sync-elassandra-routing-overlay-to-opensearch-sidecar.sh" "$DEST"
"$ROOT/scripts/sync-elassandra-essingle-node-testcase-to-opensearch-sidecar.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-routing-table-for-os13.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-shardrouting-elassandra-token-ranges.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-operation-routing-shard-routing-fix.sh" "$DEST"
"$ROOT/scripts/rewrite-elassandra-imports-for-opensearch.sh" "$DEST"
if [[ -d "$DEST/server/src/test/java/org/elassandra" ]]; then
  "$ROOT/scripts/rewrite-engine-java-for-opensearch.sh" "$DEST/server/src/test/java/org/elassandra"
  # OpenSearch renamed ESSingleNodeTestCase; package rename alone leaves a non-existent class name.
  find "$DEST/server/src/test/java/org/elassandra" -name '*.java' -print0 | xargs -0 perl -i -pe \
    's/\bESSingleNodeTestCase\b/OpenSearchSingleNodeTestCase/g'
  "$ROOT/scripts/patch-opensearch-elassandra-tests-assertions-import.sh" "$DEST"
  "$ROOT/scripts/patch-opensearch-elassandra-tests-opensearch-api.sh" "$DEST"
fi
"$ROOT/scripts/sync-mock-cassandra-discovery-to-opensearch-sidecar.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-applied-cluster-state-transport-handler.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-sourcetoparse-elassandra-token.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-fieldmapper-elassandra-createfield.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-typefieldmapper-elassandra-createfield.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-indexmetadata-elassandra-extensions.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-indexmetadata-elassandra-test-keys.sh" "$DEST"
bash "$ROOT/scripts/patch-opensearch-metadata-create-index-elassandra-keyspace.sh" "$DEST"
bash "$ROOT/scripts/patch-opensearch-cluster-task-schema-update-plumbing.sh" "$DEST"
bash "$ROOT/scripts/patch-opensearch-metadata-mapping-service-elassandra-schema.sh" "$DEST"
bash "$ROOT/scripts/patch-opensearch-metadata-delete-index-elassandra-drop.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-mapperservice-elassandra-extensions.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-elassandra-sidecar-templates.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-documentmapper-elassandra.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-documentparser-elassandra-compat.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-querymanager-opensearch-api.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-querymanager-mapping-lookup.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-querymanager-version-conflict.sh" "$DEST"
# Stubs must run before cql-process: vanilla ClusterService has no replicationFactor / SETTING tail yet.
"$ROOT/scripts/patch-opensearch-cluster-service-elassandra-stubs.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-cluster-service-elassandra-cql-process.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-elassandra-primary-first-strategy-node-id.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-metadata-elassandra-extensions.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-cluster-service-elassandra-index-settings-keys.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-index-drop-on-delete-setting.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-cluster-service-search-strategy-key.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-cluster-settings-search-strategy.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-cluster-service-elassandra-schema-stubs.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-cluster-service-elassandra-full-stubs.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-cluster-service-create-elastic-admin-keyspace.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-cluster-service-process-write-conditional.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-cluster-service-cql-clientstate.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-cluster-service-load-global-state.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-cluster-service-metadata-extensions.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-cluster-service-querymanager.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-mappers-elassandra-cql-compat.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-fieldmapper-cql-metadata-preservation.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-discovery-node-elassandra-uuid.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-mapped-field-type-elassandra-cqlvalue.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-range-fieldmapper-elassandra-compat.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-range-fieldmapper-parse-from-elassandra-fork.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-serializer-range-accessors.sh" "$DEST"
"$ROOT/scripts/overlay-elassandra-parsecontext-to-opensearch-sidecar.sh" "$DEST"
"$ROOT/scripts/install-elassandra-cluster-changed-event-opensearch.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-master-service-taskinputs-public.sh" "$DEST"
"$ROOT/scripts/patch-org-elassandra-opensearch-no-schema-update.sh" "$DEST"
"$ROOT/scripts/patch-org-elassandra-opensearch-elastic-secondary.sh" "$DEST"
bash "$ROOT/scripts/patch-org-elassandra-opensearch-elastic-query-handler-registry.sh" "$DEST"
bash "$ROOT/scripts/patch-org-elassandra-opensearch-elastic-query-handler-typeless-search.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-engine-delete-by-query.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-engine-elassandra-getresult-factory.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-gateway-service-protected-perform-recovery.sh" "$DEST"
"$ROOT/scripts/install-elassandra-gateway-opensearch-sidecar.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-gateway-module-elassandra-bind.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-node-elassandra-post-discovery-hooks.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-bootstrap-checks-allpermission-null-sm.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-discovery-module-elassandra.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-discovery-module-cluster-service-param.sh" "$DEST"
"$ROOT/scripts/patch-cassandra-discovery-for-opensearch.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-node-elassandra-activate.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-essingle-node-config-override.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-essingle-node-embedded-storage-port.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-essingle-node-threadleak.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-essingle-node-activate-createnode.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-essingle-node-mock-fs-data-path.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-opens-search-single-node-opensearch13-api.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-opens-search-single-node-nondeprecated-settings.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-essingle-node-disk-watermark.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-server-test-runtime-netty4.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-test-base-no-precreate-temp.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-test-base-delete-testrun-before-mkdir.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-test-base-skip-tmpdir-for-elassandra.sh" "$DEST"
bash "$ROOT/scripts/patch-opensearch-test-base-elassandra-test-system-properties.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-pathutils-disable-mock-for-elassandra.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-testcase-suppress-filesystems.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-testcase-threadleak-zombies-continue.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-test-base-cassandra-disable-asserts-after-ea.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-bootstrap-testing-native-scratch-dir.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-bootstrap-for-testing-elassandra-embedded-sm.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-document-parser-elassandra-delegates.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-mapping-metadata-mappers-accessor.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-metadata-field-mapper-prepost.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-datefieldmapper-elassandra-cqlvalue.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-idfieldmapper-elassandra-precreate.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-active-shard-count-cassandra-cl.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-transportshardbulkaction-elassandra-querymanager.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-transport-write-action-elassandra-dummy-location.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-indexshard-elassandra-api.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-cluster-blocks-issame.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-shard-path-resolve-snapshot.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-mapper-service-elassandra-types.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-discoverynode-inet.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-discoverynode-ephemeral-javadoc.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-index-service-keyspace.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-discovery-module-tests-opensearch13.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-readonly-engine-tests-delete-null.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-node-and-cluster-id-converter-idempotent.sh" "$DEST"
"$ROOT/scripts/patch-opensearch-logging-listener-assumption-failure.sh" "$DEST"

if [[ "${SKIP_OPENSEARCH_FORBIDDEN_DEPS_PATCH:-}" != "1" ]]; then
  "$ROOT/scripts/patch-opensearch-forbidden-deps-for-elassandra.sh" "$DEST"
fi

echo "[opensearch-sidecar-prepare] Side-car tree prepared at $DEST"
