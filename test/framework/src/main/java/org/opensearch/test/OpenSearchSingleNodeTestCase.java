...
public abstract class OpenSearchSingleNodeTestCase extends OpenSearchTestCase {
    ...
    private static boolean isCassandraDefaultUncaughtExceptionHandler(Thread.UncaughtExceptionHandler handler) {
        if (handler == null) {
            return false;
        }
        String className = handler.getClass().getName();
        return className.startsWith("org.apache.cassandra.service.CassandraDaemon")
            || className.contains("org.apache.cassandra.service.CassandraDaemon$$Lambda");
    }

    private static void snapshotDefaultUncaughtExceptionHandlerIfSafe() {
        Thread.UncaughtExceptionHandler current = Thread.getDefaultUncaughtExceptionHandler();
        if (isCassandraDefaultUncaughtExceptionHandler(current) == false) {
            uncaughtBeforeElassandraTest = current;
        }
    }
    ...
    public static synchronized void initElassandraDeamon(
        ...
        if (ElassandraDaemon.instance == null) {
            ...
            if (System.getProperty("cassandra.allow_unsafe_join") == null) {
                // Embedded single-node tests have no real peers; skip Cassandra's production shadow-round collision check.
    ...
    public static synchronized void setUpClass() throws Exception {
        snapshotDefaultUncaughtExceptionHandlerIfSafe();
        clearRandomizedZombieMarker();
    }
