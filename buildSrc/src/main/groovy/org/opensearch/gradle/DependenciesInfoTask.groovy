...
class DependenciesInfoTask extends ConventionTask {
    ...
    protected String getLicenseType(final String group, final String name) {
        ...
        if (licenseInfo.spdxLicense == false) {
            ...
            final String githubBaseURL = "https://raw.githubusercontent.com/incloudsio/elassandra/${gitBranch}/"
            final String relativeLicensePath = project.rootDir.toPath().relativize(license.toPath()).toString().replace(File.separatorChar, '/' as char)
            licenseType = "${licenseInfo.identifier};${githubBaseURL}${relativeLicensePath},"
        } else {
