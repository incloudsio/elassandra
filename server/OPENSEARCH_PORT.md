...
**Side-car test JVM (reliability + noise):** **`gradle/opensearch-sidecar-elassandra.init.gradle`** sets **`-Xms3072m -Xmx3072m`** (matching heap avoids resize pauses and BootstrapChecks warnings), **`--add-opens`** on **`java.base/java.lang`**, **`java.util`**, and **`java.util.concurrent`** (quiets JAMM **`Illegal reflective access`** lines from embedded Cassandra), plus the existing **`jdk.internal.ref`** open for Cassandra/JNA. **`ESSingleNodeTestCase`** / **`OpenSearchSingleNodeTestCase`** **`waitUntilMasterAndGatewayRecovered()`** waits for elected master, gateway recovery, **and** release of global block id **12** (no cassandra ring) before tests run; **`setUp`** calls it once per test method (after **`startNode`** when the singleton is still null). Default max wait is **5** minutes (**`-Delassandra.test.master.wait.minutes=N`**).
...
**Lucene mock FS / `java.io.tmpdir` / `FileAlreadyExistsException`:** the forked test JVM and Lucene’s test harness can disagree on temp directories (see the header comment in **`gradle/opensearch-sidecar-elassandra.init.gradle`**). Side-car patches (**`patch-opensearch-test-base-delete-testrun-before-mkdir.sh`**, **`patch-opensearch-test-base-no-precreate-temp.sh`**) reduce stale **`testrun/`** / **`temp`** issues. **`opensearch-sidecar-test-try.sh`** defaults **`OPENSEARCH_SIDECAR_TESTS_JVMS=1`** (forwards **`-Dtests.jvms=1`**) so embedded Cassandra does not contend on **`storage_port`** across parallel forks; override only if you know what you are doing. GitHub Actions side-car test workflows align with this.
